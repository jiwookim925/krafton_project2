...
- 태그는 name이 아니라 slug 기준으로 조회함 (실제 DB 유니크 제약이 slug에 걸려있어서,
  name 기준으로 찾으면 대소문자가 다를 때 "없는 줄 알고 새로 만들려다" 충돌났었음)
- transaction.atomic()으로 감싸서, 중간에 에러 나면 전부 롤백되게 함
  (부분적으로만 생성된 어중간한 데이터가 안 남도록)
"""
...
with transaction.atomic():
    ...
    author, _ = KakaoUser.objects.get_or_create(
        kakao_id=1000000001,
        defaults={"nickname": "테스트유저", "profile_image": None, "created_at": timezone.now()},
>>>>>>> 7fc83b88d93b0106879eabfaf1d64e128d756bf5
    )
    ...
    # 3) 태그 - slug로 조회 (name으로 조회하면 대소문자 달라졌을 때 중복 생성 시도하다 에러남)
    tag_names = ["Django", "Next.js", "Python", "TypeScript", "회고"]
    tags = {}
    for name in tag_names:
